...
export async function generateStaticParams() {
  const modules = getModules("cloud-pm");
  return modules.map((m) => ({ slug: m.slug }));
}

export default async function ModulePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const mod = getModule("cloud-pm", slug);
  if (!mod) notFound();

  const allModules = getModules("cloud-pm");
  ...
  return (
    <main className="min-h-screen px-6 py-16" style={{ background: "var(--cream)" }}>
      <div className="max-w-2xl mx-auto space-y-10">
        ...
        <DeliverableForm moduleId={mod.id} track="cloud-pm" nextSlug={nextModule?.slug ?? null} />
      </div>
    </main>
  );
}
